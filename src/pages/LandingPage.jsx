...
const navigation = [['experiencia', 'Experiencia'], ['proyectos', 'Proyectos'], ['enfoque', 'Enfoque']];
function Tags({
  items
}) {
...
function ProjectCard({
  ...
  return <article className={`work-card ${project.color}`}>
    ...
    <div className="work-meta"><span>{project.label}</span></div>
...
function DemoPreview({ mode, sizing = false }) {
  return <div className={`demo-window ${mode}${sizing ? " demo-sizer" : ""}`} aria-hidden={sizing || undefined}>
        ...
        <div className="demo-body"><div className="demo-nav"><strong>forma®</strong><span>OBJETOS COTIDIANOS</span><span>☰</span></div><div className="demo-product"><span className="demo-index">DISEÑO RESPONSIVE</span><div className="sculpture"><div className="sculpture-ring" /><div className="sculpture-base" /></div><span className="demo-caption">Menos, pero mejor.</span></div><div className="demo-bottom"><div><strong>Diseño con intención.</strong><span>Una exploración de UI responsive.</span></div><span className="demo-round">↗</span></div></div>
      </div>;
}
function InterfaceStudy() {
  const [mode, setMode] = useState('desktop');
  return <div className="interface-study">
    <div className="study-top"><span><span className="live-dot" /> DEMO INTERACTIVA</span><span>REACT · UI</span></div>
...
export default function LandingPage() {
  ...
  return <>
    ...
    <main id="contenido">
      ...
      <section className="experience-section section-wrap" id="experiencia" aria-labelledby="experience-heading"><div className="experience-intro"><span className="eyebrow">EXPERIENCIA PROFESIONAL</span><h2 id="experience-heading">Experiencia que<br />da <em>perspectiva.</em></h2><p>Del desarrollo de sistemas internos a aplicaciones empresariales y e-commerce. Una evolución hacia mi especialización en frontend.</p><a className="text-link" href={cv} target="_blank" rel="noreferrer">CV en español <FaArrowUpRightFromSquare /></a><a className="text-link" href={cvEnglish} target="_blank" rel="noreferrer" lang="en">Resume in English <FaArrowUpRightFromSquare /></a></div><div className="timeline">{experience.map(job => <article className="job" key={job.company}><div className="job-top"><span>{job.period}</span>{job.current && <span className="current-label"><span className="live-dot" /> ACTUAL</span>}</div><h3>{job.company}</h3><p className="job-role">{job.role} <span> / {job.specialty}</span></p><p className="job-description">{job.text}</p><ul className="job-bullets">{job.bullets.map(bullet => <li key={bullet}>{bullet}</li>)}</ul><Tags items={job.stack} /></article>)}<details className="earlier-experience"><summary>Experiencia inicial · Desarrollo web y soporte TI <FaPlus className="plus" /><FaMinus className="minus" /></summary><div><h3>LSR Distribuidor</h3><span>Web Developer & IT Support · Sep. 2023 — Abr. 2024</span><p>Desarrollé y mantuve cuatro sitios web empresariales con WordPress, Elementor, HTML y CSS. Personalicé la estructura de las páginas, los estilos y su adaptación a distintos tamaños de pantalla.</p><p>Combiné este trabajo con soporte a usuarios, mantenimiento preventivo y correctivo de equipos, instalación de sistemas y software, cableado y conectividad de red. También gestioné cuentas de correo corporativo y la conexión de estaciones de trabajo con los servidores de la empresa.</p><h3>Robuspack</h3><span>Full Stack Developer Intern · Ago. 2023 — Ene. 2024</span><p>Contribuí al desarrollo de un sistema automatizado de gestión de inventarios durante mis prácticas profesionales. Construí y mantuve interfaces web con HTML, Bootstrap y JavaScript, y participé en la funcionalidad backend con PHP.</p><p>Colaboré con el equipo en el diseño, implementación y pruebas de nuevas funciones para los procesos de inventario, trabajando tanto en la interfaz como en la lógica de la aplicación.</p></div></details></div></section>
      ...
      <section className="work-section section-wrap" id="proyectos" aria-labelledby="work-heading"><div className="section-heading"><div><span className="eyebrow">PROYECTOS PERSONALES · CÓDIGO ABIERTO</span><h2 id="work-heading">Ideas que ya<br />puedes <em>explorar.</em></h2></div><p>Proyectos personales, decisiones de interfaz<br className="desktop-break" /> y código abierto. Mira cómo están construidos.</p></div>
        ...
        <div className="work-grid">{visibleProjects.map(project => <ProjectCard project={project} key={project.id} />)}</div>
      ...
      <section className="approach-section" id="enfoque" aria-labelledby="approach-heading"><div className="section-wrap"><div className="section-heading"><div><span className="eyebrow">ESPECIALIDADES</span><h2 id="approach-heading">La interfaz es donde<br />todo <em>conecta.</em><span className="orange-star">✳</span></h2></div><p>Frontend como especialidad.<br />Visión de producto en cada decisión.</p></div><div className="capabilities">
        {[['INTERFACES WEB', 'Frontend engineering', 'Interfaces que se sienten naturales.', 'React, Next.js y TypeScript para construir componentes reutilizables, layouts responsive y experiencias con atención a accesibilidad y web performance.', ['React', 'Next.js', 'TypeScript', 'HTML & CSS']], ['EXPERIENCIA DE COMPRA', 'E-commerce & CRO', 'Cada interacción tiene una intención.', 'Storefronts en Shopify, páginas de producto, colecciones y carritos. Implementación de diseños de Figma y experimentos A/B para trabajar sobre la fricción de compra.', ['Shopify', 'Liquid', 'Figma', 'UX & CRO']], ['INTEGRACIONES Y DATOS', 'Ingeniería de producto', 'Una buena interfaz necesita una buena base.', 'Mi experiencia full stack me permite entender APIs, datos e integraciones, colaborar con backend y acompañar el software hasta su entrega.', ['REST APIs', 'Node.js', 'C# / .NET', 'Git & CI/CD']]].map(([label, title, subtitle, text, tags]) => <article className="capability" key={title}><span className="capability-label">{label}</span><h3>{title}</h3><h4>{subtitle}</h4><p>{text}</p><Tags items={tags} /></article>)}
